...
A \textbf{Graph} can be understood as a collection of Vertices which are
connected to each other by Edges.  A \textbf{Vertex} can be understood as a
point and an \textbf{Edge} is a pair of Vertices.  The set of all the vertices
in a graph $G$ can be represented as \textbf{$V(G)$} and the set of all the
edges in $G$ is represented as \textbf{$E(G)$}.

For a vertex $v$, it's degree \textbf{$deg(v)$} is the number of edges
connected to it.  An \textbf{isolated vertex} $v$ is such that $deg(v) = 0$. An
\textbf{end vertex} is $w$ such that $deg(w) = 1$.  Two vertices are
\textbf{adjacent} to each other if there is an edge connects them.

A \textbf{bipartite} graph, is a graph $G$ such that it's vertices $V(G)$ can
